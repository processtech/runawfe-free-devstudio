...
public class VariableTypeEditorPage extends EditorPartBase<VariableUserType> {
    ...
    private class RemoveTypeSelectionListener extends LoggingSelectionAdapter {
        ...
        protected void onSelection(SelectionEvent e) throws Exception {
            ...
            for (VariableUserType userType : getDefinition().getVariableUserTypes()) {
                // ƒанна€ проверка выполн€етс€ снаружи метода, чтобы избежать ситуации сравнени€ типа с самим собой.
    ...
    private class PasteAttributeSelectionListener extends LoggingSelectionAdapter {
        ...
        /**
         * —транный метод сравнени€ типов.
         * ћетод выделен дл€ реализации сравнени€ типов, не затрагива€ базовые классы. ≈сли писать в них, то можно уйти в StackOverflowError.
         * @param leftType “ип дл€ сравнени€ слева от знака равенства.
         * @param rightType “ип дл€ сравнени€ справа от знака равенства.
         * @return ѕризнак равенства типов.
         */
        private boolean isEquals(VariableUserType leftType, VariableUserType rightType) {
            ...
            if (leftType.getAttributes() != null && rightType.getAttributes() != null) {
                // не должно быть result && поскольку предыдуща€ проверка установит false _не верно_ при переходе сюда.
