...
import ru.runa.gpd.editor.graphiti.update.ChangeCycleTypeFeature;
...
public class DiagramToolBehaviorProvider extends DefaultToolBehaviorProvider {
    ...
    public IContextButtonPadData getContextButtonPad(IPictogramElementContext context) {
        ...
        if ((element instanceof MessageNode) || (element instanceof EmbeddedSubprocess)) {
            ContextButtonEntry changeEventTypeButton = new ContextButtonEntry(null, null);
            changeEventTypeButton.setIconId("wrench.png");
            data.getDomainSpecificContextButtons().add(changeEventTypeButton);
            PictogramElement pes[] = { pe };
            ...
            if (element instanceof MessageNode) {
            	...
            	changeEventTypeButton.setText(Localization.getString("cycle.type.label"));
            	changeEventTypeButton.setDescription(Localization.getString("cycle.type.description"));
            	for (int i = 0; i < CycleType.LABELS.length; i++) {
            		CycleType ct = CycleType.values()[i];
            		ContextButtonEntry createButton = new ContextButtonEntry(new ChangeCycleTypeFeature(getFeatureProvider(), ct), customContext);
            		// createButton.setIconId("cycle/" + ct.getImageName());
            		createButton.setText(CycleType.LABELS[i]);
            		changeEventTypeButton.addContextButtonMenuEntry(createButton);
            	}
            }
        }
