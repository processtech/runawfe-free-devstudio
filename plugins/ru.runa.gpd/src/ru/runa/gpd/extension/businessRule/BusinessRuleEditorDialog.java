...
public class BusinessRuleEditorDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (GroovyStuff.TYPE.getAll().size() > 0) {
			SwtUtils.createLink(sourceHeader, Localization.getString("Insert.TYPE.link"),
		...
		if (GroovyStuff.CONSTANT.getAll().size() > 0) {
			SwtUtils.createLink(sourceHeader, Localization.getString("Insert.CONSTANT.link"),
					new LoggingHyperlinkAdapter() {
						@Override
						public void onLinkActivated(HyperlinkEvent e) {
		...
		if (GroovyStuff.STATEMENT.getAll().size() > 0) {
			SwtUtils.createLink(sourceHeader, Localization.getString("Insert.STATEMENT.link"),
					new LoggingHyperlinkAdapter() {
						@Override
						public void onLinkActivated(HyperlinkEvent e) {
		...
		if (GroovyStuff.METHOD.getAll().size() > 0) {
			SwtUtils.createLink(sourceHeader, Localization.getString("Insert.METHOD.link"),
					new LoggingHyperlinkAdapter() {
						@Override
						public void onLinkActivated(HyperlinkEvent e) {
		...
		SwtUtils.createLink(sourceHeader, Localization.getString("button.insert_variable"),
				new LoggingHyperlinkAdapter() {
					@Override
	...
	private Composite createButtomComposite() {
		...
		defaultButton.addSelectionListener(new LoggingSelectionAdapter() {
			...
			protected void onSelection(SelectionEvent e) throws Exception {
				FormulaCellEditorProvider.ConfigurationDialog dialog = new FormulaCellEditorProvider.ConfigurationDialog(defaultButton.getText(), variableNames);
	...
	private void createLines(String function) {
		...
		functionButton.addSelectionListener(new LoggingSelectionAdapter() {
			@Override
			protected void onSelection(SelectionEvent e) throws Exception {
				FormulaCellEditorProvider.ConfigurationDialog dialog = new FormulaCellEditorProvider.ConfigurationDialog(functionButton.getText(), variableNames);
				if (dialog.open() == Window.OK) {
	...
	private void toCode() {
		for (int i = 0; i < varBoxes.size(); i++) {
			if (varBoxes.get(i)[0].getText().length() == 0 || varBoxes.get(i)[1].getText().length() == 0
					&& !buttons.get(i).getText().equals(defaultFunction)) {
		...
		try {
			...
			for (int i = 0; i < functions.size(); i++) {
				...
				GroovyTypeSupport typeSupport = GroovyTypeSupport.get(variable1.getJavaClassName());
				ifExpr = new IfExpr(buttons.get(i).getText(), variable1, lexem2,
						Operation.getByName(operationName, typeSupport));
